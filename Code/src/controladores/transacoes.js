let { contas, saques, depositos, transfereicas, transferencias } = require('../bancodedados');
...
const sacar = (req, res) => {
    ...
    if (!numero_conta || !valor || !senha) {
        return res.status(400).json({ mensagem: 'O número da conta e/ou o valor e/ou valor são obrigatórios.' });
    }
    const contaEncontrada = contas.find(conta => Number(conta.numero) === Number(numero_conta));
...
const transferir = (req, res) => {
    ...
    if (!numero_conta_origem || !numero_conta_destino || !valor || !senha) {
        return res.status(400).json({ mensagem: 'O número da conta de origem e/ou destino e/ou o valor e/ou valor são obrigatórios.' });
    }
